...
class ExtendedTuringMachine(TuringMachine):
    ...
    def generar_cinta_aleatoria(self, largo=50):
        ...
        print(f"Cinta generada: {''.join(s for s in self.tape)}") # imprimir la cinta limpia
...
def mostrar_cinta_segmentada(maquina):
    print("\nCinta:")

    posiciones = [str(i) for i in range(len(maquina.tape))]
    cinta = maquina.tape

    # Dividir la cinta en segmentos de 10 elementos
    for i in range(0, len(cinta), 10):
        # Crear una nueva tabla para cada segmento
        table = PrettyTable(header=False)
        
        # Agregar la fila de posiciones y valores para el segmento actual
        table.add_row(posiciones[i:i + 10])
        table.add_row(cinta[i:i + 10])

        # Imprimir la tabla del segmento actual
        print(table)
...
def menu():
    ...
    while True:
        ...
        if opcion == '1':
            ...
            mostrar_cinta_segmentada(maquina)
